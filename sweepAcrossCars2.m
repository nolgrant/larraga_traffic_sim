function sweepAcrossCars2(R_in, a_in, string_in, plotstr, L, T_burnin)
...
for iteration = 1:length(R_in)
    ...
    for denisty_ix = 1:length(N_values)
        ...
        for t = 1:T-1 %iterate over all time steps
            ...
            % S1: Acceleration & S2: Randomization
            % (each car's tentative velocity depends only on itself, so
            % these can be computed independently of neighbors)
            ...
            % S3: Deceleration, iterated using neighbors' tentative
            % (post S1/S2) velocities, which are updated further as the
            % correction passes propagate braking down a chain of cars
            for correction_pass = 1:(vmax)
                changed = false;
                for n = 1:N
                    next_closest_ix = sort_idx((find(n == sort_idx,1) + 1));
                    xp = all_cars_x(next_closest_ix);   % positions haven't moved yet
                    vp = v_next_vec(next_closest_ix);   % neighbor's UPDATED velocity
                    x_curr = all_cars_x(n);
                    if x_curr < xp
                        d = xp - x_curr - 1;
                    else
                ...
                if ~changed
                    break   % converged early -- most timesteps won't need all vmax-1 passes
                end
            end
